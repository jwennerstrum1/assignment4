...
public class TrapPainter implements StaticPainter {
    ...
    public void paint(Graphics2D g2, State s, float cWidth, float cHeight) {
        ...
        for(int i = 0; i < BasicRewardFunction.trapLocations.length; i++){
//            for(int j = 0; j < trapLocations[0].length; j++){

//                //is there a wall here?
//                if(this.map[i][j] == 1){

                    //left coordinate of cell on our canvas
                    float rx = BasicRewardFunction.trapLocations[i][0] *width;

                    //top coordinate of cell on our canvas
                    //coordinate system adjustment because the java canvas
                    //origin is in the top left instead of the bottom right
                    float ry = cHeight - height - BasicRewardFunction.trapLocations[i][1]*height;
